...
def setCity(user_id,dirtyCityName):
    try:
        cityName=(requests.get('https://maps.googleapis.com/maps/api/geocode/json?address='+dirtyCityName+'&key=*Google API key*&language=ru').json()['results'])[0]['address_components'][0]['long_name']
...
def settings(message):
    ...
    try:
        ...
        if dict.get(status, message.chat.id) == None or dict.get(status, message.chat.id) == 0:
            bot.send_message(message.chat.id, 'Укажи город (можно неточно):')
